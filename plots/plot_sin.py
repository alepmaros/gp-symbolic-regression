...
for i, scores_path in enumerate(args.scores):
    with open(scores_path, 'r') as fhandle:
        ...
        allow_sin = score['Parameters']['Allow Sin']
        if (allow_sin):
            label = 'Com função seno'
        else:
            label = 'Sem função seno'
...
save_path = os.path.join(save_path, '{}_allow_sin_train.pdf'.format(args.dataset))
plt.tight_layout()
...
# save_path, _ = os.path.split(args.scores[0])
# save_path = os.path.join(save_path, '{}_allow_sin_best_test.pdf'.format(args.dataset))
# plt.tight_layout()
# plt.savefig(save_path, format='pdf')


save_path, _ = os.path.split(args.scores[0])
save_path = os.path.join(save_path, '{}_allow_sin_best.pdf'.format(args.dataset))
plt.tight_layout()
plt.savefig(save_path, format='pdf')